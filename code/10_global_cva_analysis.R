list.of.packages <- c("data.table","tidyverse", "xlsx", "stringr", "stringdist")
...
sub_grants_agg = data.table(sub_grants)[
  ,.(CVAamount_subgrant=sum(Amount.USD, na.rm=T)),
  by=.(clean_org, Year)
]
sub_grants_agg = subset(sub_grants_agg, !is.na(clean_org))
setnames(sub_grants_agg, "Year", "year")

# Merge and subtract
cva_agg = merge(cva_agg, sub_grants_agg, by=c("clean_org", "year"), all.x=T)
cva_agg$CVAamount_subgrant[which(is.na(cva_agg$CVAamount_subgrant))] = 0
cva_agg$CVAamount_undoubled = cva_agg$CVAamount - cva_agg$CVAamount_subgrant
cva_agg$CVAamount_undoubled = pmax(cva_agg$CVAamount_undoubled, 0)
cva_agg_org_type = cva_agg[,.(CVAamount=sum(CVAamount_undoubled)), by=.(year, cva_org_type)]
