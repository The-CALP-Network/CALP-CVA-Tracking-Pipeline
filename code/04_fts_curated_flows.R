### Does this work Function to download and curate FTS flows from the FTS API
...
fts_curated_flows <- function(years = 2017:2024, update_years = NA, dataset_path = "fts", base_year = 2022, weo_ver = NULL) {
  ...
  fts <- merge(fts, deflators, by = c("source_org_iso3", "year"), all.x = TRUE, sort = FALSE, suffixes = c("", ".defl"))
  fts[, deflator := fifelse(is.na(deflator), get("deflator.defl"), deflator)]
