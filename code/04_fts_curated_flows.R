...
fts_curated_flows <- function(years = 2017:2024, update_years = NA, dataset_path = "fts", base_year = 2022, weo_ver = NULL, dummy_intra_country_flows = T){
  ...
  source_org_dicode <- fread("reference_datasets/source_orgs_DIcode.csv", encoding = "UTF-8", showProgress = F)
  ...
  destination_org_dicode <- fread("reference_datasets/destination_orgs_DIcode.csv", encoding = "UTF-8", showProgress = F)
  ...
  dac_and_eu = fread("reference_datasets/dac_and_eu.csv")
  eu = subset(dac_and_eu, source_orgtype=="Multilateral: EC")
  euc_id <- as.character(unique(eu$sourceObjects_Organization.id))
  ...
  cluster_dicode <- fread("reference_datasets/cluster_mapping_DIcode.csv", showProgress = F)
